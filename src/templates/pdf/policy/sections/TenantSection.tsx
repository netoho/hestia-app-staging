...
import type { PDFPolicyData, PDFTenant } from '@/lib/pdf/types';
  ...
  if (!tenants || tenants.length === 0) {
    return (
      <View style={styles.section}>
        <SectionTitle title="Inquilino(s)" />
        <View style={styles.emptyState}>
          <Text>No hay inquilinos registrados</Text>
        </View>
      </View>
    );
  }
    ...
    <View style={styles.section}>
      ...
      <View style={styles.sectionContent}>
        {tenants.map((tenant, index) => (
          <TenantCard key={index} tenant={tenant} index={index} total={tenants.length} />
        ))}
      </View>
    </View>
  );
...
function TenantCard({ tenant, index, total }: TenantCardProps) {
  // Neutral numbering only when there are co-tenants — never "Principal".
  const titlePrefix = total > 1 ? `Inquilino ${index + 1} — ` : '';
      ...
      <View style={styles.card} wrap={false}>
        <View style={styles.cardHeader}>
          <Text style={styles.cardTitle}>
            {titlePrefix}{tenant.isCompany ? 'Empresa' : 'Persona Física'}: {tenant.name}
          </Text>
