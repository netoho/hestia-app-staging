...
export default function TenantPortalPage({
  ...
  const [formData, setFormData] = useState<TenantData>({
    ...
    previousRentAmount: 0,
    previousRentalAddress: '',
    rentalHistoryYears: 0,
    ...
    try {
      ...
      if (data.tenant) {
        setFormData({
          ...formData,
          ...data.tenant,
          addressDetails: data.tenant.addressDetails || null,
        });

        // Load references
        if (data.tenant.references && data.tenant.references.length > 0) {
          setReferences(data.tenant.references);
        }

        // Load commercial references if company
        if (data.tenant.commercialReferences && data.tenant.commercialReferences.length > 0) {
          setCommercialReferences(data.tenant.commercialReferences);
        }
      }
  ...
  const validateForm = (): boolean => {
    ...
    // Validate references
    const refsToCheck = formData.tenantType === 'COMPANY' ? commercialReferences : references;
    const refType = formData.tenantType === 'COMPANY' ? 'comerciales' : 'personales';
  ...
  const handleSave = async () => {
    ...
    try {
      const submitData = {
        ...formData,
        references: formData.tenantType === 'INDIVIDUAL' ? references : undefined,
        commercialReferences: formData.tenantType === 'COMPANY' ? commercialReferences : undefined,
      ...
      if (!response.ok) {
        ...
        throw new Error(error.message || 'Failed to save tenant information');
      }
  ...
  const tabs = formData.tenantType === 'INDIVIDUAL'
    ? [
        { id: 'personal', label: 'Información Personal', icon: User },
        { id: 'employment', label: 'Información Laboral', icon: Briefcase },
        { id: 'rental', label: 'Historial de Renta', icon: Home },
      ...
      <Card className="max-w-4xl mx-auto">
        ...
        <CardContent>
          ...
          <div className="mb-6 p-4 border rounded-lg">
            <Label className="text-base font-medium mb-3 block">Tipo de Inquilino</Label>
            <RadioGroup
              ...
              <>
                    ...
                    <div className="md:col-span-2">
                      <AddressAutocomplete
                        ...
                        onChange={(address) => {
                          ...
                          updateFormData('currentAddress', address.formattedAddress ||
                ...
                <TabsContent value="employment" className="space-y-4 mt-4">
                  ...
                  <div className="flex justify-between">
                    ...
                    <Button onClick={() => setCurrentTab('rental')}>
                      Siguiente
                    </Button>
                  </div>
                </TabsContent>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => updateFormData('previousLandlordEmail', e.target.value)}
                        placeholder="correo@ejemplo.com"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => updateFormData('previousRentAmount', parseFloat(e.target.value) || 0)}
                        placeholder="0.00"
                        min="0"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => updateFormData('rentalHistoryYears', parseInt(e.target.value) || 0)}
                        placeholder="0"
                        min="0"
                      />
                    </div>
                    ...
                    <div className="md:col-span-2">
                      <Label htmlFor="previousRentalAddress">Dirección de Renta Anterior</Label>
                      <Input
                        id="previousRentalAddress"
                        value={formData.previousRentalAddress}
                        onChange={(e) => updateFormData('previousRentalAddress', e.target.value)}
                        placeholder="Dirección completa"
                      />
                    </div>
                  </div>
                ...
                <TabsContent value="references" className="space-y-4 mt-4">
                  ...
                  <div className="flex justify-between">
                    <Button variant="outline" onClick={() => setCurrentTab('rental')}>
                      Anterior
                    </Button>
                    ...
                    <div className="md:col-span-2">
                      <div className="flex items-center space-x-2">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => updateFormData('requiresCFDI', checked)}
                        />
                        <Label htmlFor="requiresCFDI">Requiere factura (CFDI)</Label>
                      </div>
                    </div>
                  </div>
              ...
              <div className="flex justify-between">
                <Button
                  ...
                  onClick={() => setCurrentTab(
                    formData.tenantType === 'INDIVIDUAL' ? 'references' : 'references'
                  )}
                >
                  Anterior
                </Button>
