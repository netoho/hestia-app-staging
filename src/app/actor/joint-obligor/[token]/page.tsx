...
import { Save, CheckCircle2, AlertCircle, User, Briefcase, Building, Home, FileText, Users, XCircle, Loader2 } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import { AddressAutocomplete } from '@/components/forms/AddressAutocomplete';

interface JointObligorData {
  // Type
  obligorType: 'INDIVIDUAL' | 'COMPANY';
...
export default function JointObligorPortalPage({
  ...
  // Fetch joint obligor and policy data
  useEffect(() => {
    if (token) {
  ...
  const validateTokenAndLoadData = async () => {
    try {
      ...
      // If joint obligor data exists, populate the form
      if (data.jointObligor) {
        setFormData({
          ...formData,
          ...data.jointObligor,
          addressDetails: data.jointObligor.addressDetails || null,
        });

        // Load references
        if (data.jointObligor.references && data.jointObligor.references.length > 0) {
          setReferences(data.jointObligor.references);
        }

        // Load commercial references if company
        if (data.jointObligor.commercialReferences && data.jointObligor.commercialReferences.length > 0) {
          setCommercialReferences(data.jointObligor.commercialReferences);
        }
      }
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.obligorType === 'INDIVIDUAL') {
  ...
  const handleSave = async () => {
    ...
    try {
      const submitData = {
        ...formData,
        references: formData.obligorType === 'INDIVIDUAL' ? references : undefined,
        commercialReferences: formData.obligorType === 'COMPANY' ? commercialReferences : undefined,
      ...
      if (!response.ok) {
        ...
        throw new Error(error.message || 'Failed to save joint obligor information');
      }
  ...
  const tabs = formData.obligorType === 'INDIVIDUAL'
    ? [
        { id: 'personal', label: 'Información Personal', icon: User },
        { id: 'employment', label: 'Información Laboral', icon: Briefcase },
        { id: 'financial', label: 'Información Financiera', icon: Building },
      ...
      <Card className="max-w-4xl mx-auto">
        ...
        <CardContent>
          ...
          <div className="mb-6 p-4 border rounded-lg">
            <Label className="text-base font-medium mb-3 block">Tipo de Obligado Solidario</Label>
            <RadioGroup
              value={formData.obligorType}
              onValueChange={(value) => {
                updateFormData('obligorType', value);
          ...
          <Tabs value={currentTab} onValueChange={setCurrentTab}>
            ...
            {/* INDIVIDUAL TABS */}
            {formData.obligorType === 'INDIVIDUAL' && (
              <>
                ...
                <TabsContent value="employment" className="space-y-4 mt-4">
                  ...
                  <div className="flex justify-between">
                    ...
                    <Button onClick={() => setCurrentTab('financial')}>
                      Siguiente
                    </Button>
                  </div>
                </TabsContent>
                ...
                <TabsContent value="financial" className="space-y-4 mt-4">
                  <Alert>
                    <AlertDescription>
                      Esta información es opcional pero puede fortalecer su solicitud.
                    </AlertDescription>
                  </Alert>
                    ...
                    <div className="md:col-span-2">
                      <div className="flex items-center space-x-2 mb-4">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => updateFormData('hasProperties', checked)}
                        />
                        <Label htmlFor="hasProperties">¿Cuenta con propiedades inmuebles?</Label>
                      </div>

                      {formData.hasProperties && (
                        <div>
                          <Label htmlFor="propertyValue">Valor Aproximado de Propiedades (MXN)</Label>
                          <Input
                            id="propertyValue"
                            type="number"
                ...
                <TabsContent value="references" className="space-y-4 mt-4">
                  ...
                  <div className="flex justify-between">
                    <Button variant="outline" onClick={() => setCurrentTab('financial')}>
                      Anterior
                    </Button>
            ...
            {formData.obligorType === 'COMPANY' && (
              <>
                ...
                <TabsContent value="financial" className="space-y-4 mt-4">
                    ...
                    <div className="md:col-span-2">
                      <div className="flex items-center space-x-2 mb-4">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => updateFormData('hasProperties', checked)}
                        />
                        <Label htmlFor="hasProperties">¿La empresa cuenta con propiedades inmuebles?</Label>
                      </div>
                  ...
                  <div className="flex justify-between">
                    <Button variant="outline" onClick={() => setCurrentTab('representative')}>
                      Anterior
                    </Button>
                    <Button onClick={() => setCurrentTab('references')}>
                      Siguiente
                  ...
                  <CardContent>
                    {formData.obligorType === 'INDIVIDUAL' ? (
                      <ul className="space-y-2 text-sm">
                        ...
                        <li>• Escrituras de propiedades (si aplica)</li>
                      </ul>
                    ) : (
                      <ul className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                <Button
                  ...
                  onClick={() => setCurrentTab(
                    formData.obligorType === 'INDIVIDUAL' ? 'references' : 'references'
                  )}
                >
                  Anterior
                </Button>
