...
export async function DELETE(
  ...
  try {
    ...
    // Verify that the document belongs to the specified aval
    ...
    if (!document) {
      return NextResponse.json(
        { error: 'Document not found or does not belong to this aval' },
        { status: 404 }
      );
    }
    ...
    await prisma.policyActivity.create({
      data: {
        policyId,
        action: 'aval_document_deleted',
        description: `Documento de aval eliminado: ${document.originalName}`,
        details: {
          actorId,
          documentId,
