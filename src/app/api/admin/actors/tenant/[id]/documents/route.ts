/**
 * Admin endpoint for tenant document management
  ...
  try {
    ...
    await prisma.policyActivity.create({
      data: {
        ...
        description: `Admin uploaded ${documentType} document for tenant`,
...
export async function GET(
  ...
  try {
    ...
    const formattedDocuments = tenant.documents.map(doc => ({
      id: doc.id,
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        totalCount: tenant.documents.length,
      },
    });
  } catch (error) {
  ...
  try {
    ...
    await prisma.policyActivity.create({
      data: {
        ...
        description: `Admin deleted tenant document`,
        performedById: userId,
        details: {
          documentId,
