/**
 * Admin endpoint for joint-obligor document management
  ...
  try {
    ...
    await prisma.policyActivity.create({
      data: {
        ...
        description: `Admin uploaded ${documentType} document for joint obligor`,
...
export async function GET(
  ...
  try {
    ...
    const formattedDocuments = jointObligor.documents.map(doc => ({
      id: doc.id,
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        totalCount: jointObligor.documents.length,
      },
    });
  } catch (error) {
    console.error('Error fetching joint obligor documents:', error);
    return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    ...
    // 2. Get joint-obligor and verify document ownership
    const jointObligor = await prisma.jointObligor.findUnique({
      where: { id: jointObligorId },
      include: {
        documents: {
    ...
    if (jointObligor.documents.length === 0) {
      return NextResponse.json(
    ...
    await prisma.policyActivity.create({
      data: {
        ...
        description: `Admin deleted joint obligor document`,
        performedById: userId,
        details: {
          documentId,
