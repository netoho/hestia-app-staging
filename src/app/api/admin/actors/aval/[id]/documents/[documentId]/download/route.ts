/**
 * Admin endpoint for aval document download
...
export async function GET(
  ...
  try {
    ...
    const { id: avalId, documentId } = await params;
    ...
    const document = aval.documents[0];
    if (!document) {
      return NextResponse.json(
    ...
    await prisma.policyActivity.create({
      data: {
        ...
        description: `Admin downloaded ${document.documentType} document for aval`,
