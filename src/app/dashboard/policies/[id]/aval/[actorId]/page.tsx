...
export default function AvalEditPage({
  params
}: {
  ...
  const fetchAvalData = async () => {
    try {
      const response = await fetch(`/api/policies/${policyId}/aval/${actorId}`);
      if (!response.ok) throw new Error('Failed to fetch aval');

      const data = await response.json();

      if (data.success && data.data) {
        const aval = data.data;
        setFormData({
          fullName: aval.fullName || '',
          email: aval.email || '',
          phone: aval.phone || '',
          nationality: aval.nationality || 'MEXICAN',
          curp: aval.curp || '',
          rfc: aval.rfc || '',
          ...
          monthlyIncome: aval.monthlyIncome || 0,
          incomeSource: aval.incomeSource || '',
          propertyAddress: aval.propertyAddress || '',
          propertyValue: aval.propertyValue || 0,
          propertyDeedNumber: aval.propertyDeedNumber || '',
          propertyRegistry: aval.propertyRegistry || '',
          informationComplete: aval.informationComplete || false,
          additionalInfo: aval.additionalInfo || '',
        });
        setReferences(aval.references || []);
        setPolicyInfo({
          policyNumber: data.policyNumber || '',
          propertyAddress: data.policyPropertyAddress || ''
        });
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    if (!validateForm()) {
      ...
      } else if (errorKeys.some(key => ['propertyAddress', 'propertyValue'].includes(key))) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || 'Failed to save aval information');
      }
          ...
          <Tabs value={currentTab} onValueChange={setCurrentTab}>
            ...
            <TabsContent value="property" className="space-y-4 mt-4">
              <Alert>
                ...
                <AlertDescription>
                  Información sobre la propiedad que se ofrece como garantía
                </AlertDescription>
              </Alert>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2">
                  ...
                  <Input
                    ...
                    placeholder="Dirección completa de la propiedad en garantía"
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder="Número de escritura (opcional)"
                  />
                </div>

                <div className="md:col-span-2">
                  <Label htmlFor="propertyRegistry">Registro de la Propiedad</Label>
                  <Input
                    id="propertyRegistry"
                    value={formData.propertyRegistry}
                    onChange={(e) => updateFormData('propertyRegistry', e.target.value)}
                    placeholder="Datos del registro público (opcional)"
                  />
                </div>
              </div>
